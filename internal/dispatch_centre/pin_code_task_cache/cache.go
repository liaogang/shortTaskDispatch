...
func (slf *Cache) DispatchAndWaitFinish(pinCode string, item *Task.Item, timeout time.Duration) ([]byte, error) {
	...
	log.Trace().Str("pinCode", pinCode).Interface("发布任务", item).Send()

	val, ok := slf.claimBufChannel.LoadAndDelete(pinCode)
	if !ok {
		log.Trace().Str("pinCode", pinCode).Msg("no client for this pinCode waiting")
		return nil, fmt.Errorf("no client for this pinCode waiting")
	}

	var channel = val.(chan *Task.Item)

	log.Trace().Str("pinCode", pinCode).Msg("等待任务被认领")
	channel <- item

	log.Trace().Str("pinCode", pinCode).Msg("任务已被认领, 等待任务完成")
	...
	select {
	case <-t.C:
		log.Trace().Str("pinCode", pinCode).Msg("超时")
		slf.mFinishChannel.Delete(item.Id)
		return nil, fmt.Errorf("timeout")
	case wrap := <-finishChannel:
		log.Trace().Str("pinCode", pinCode).Msg("已完成")
...
func (slf *Cache) ClaimAndWait(pinCode string) (*Task.Item, error) {
	...
	log.Trace().Str("pinCode", pinCode).Msg("认领任务")
	...
	select {
	case <-t.C:
		log.Trace().Str("pinCode", pinCode).Msg("认领任务超时")
		slf.claimBufChannel.Delete(pinCode)
		return nil, fmt.Errorf("timeout")
	case taskItem := <-channel:
		log.Trace().Str("pinCode", pinCode).Msg("已认领到一个任务")
		return taskItem, nil
	}
...
func (slf *Cache) Finish(id string, payload []byte, err error) error {
	...
	log.Trace().Str("taskId", id).Msg("完成任务")

	if val, ok := slf.mFinishChannel.LoadAndDelete(id); ok {

		var channel = val.(chan Wrap)

		log.Trace().Str("taskId", id).Msg("发送给发布方")
		channel <- Wrap{
			err:     err,
			payload: payload,
		}

		log.Trace().Str("taskId", id).Msg("对方已接收")
